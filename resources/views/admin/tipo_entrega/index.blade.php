...
@section('title', 'Tipos de Entrega — Carga y Logística Tolima')
    ...
    @if (session('error'))
        <div class="alert alert-premium-danger alert-dismissible fade show mt-2" role="alert">
            <div class="d-flex align-items-center">
                ...
                <div class="text-white font-weight-medium">{{ session('error') }}</div>
            </div>
            <button type="button" class="close text-white" data-dismiss="alert" aria-label="Close" style="opacity: 0.5; background: transparent; border: none;">
                <span aria-hidden="true">&times;</span>
            </button>
        ...
        <div class="col-xl-4 col-lg-5 col-12 mb-4">
            <div class="card-premium-box">
                ...
                <div class="card-body p-4">
                    <form action="{{ route('admin.tipo-entrega.store') }}" method="POST" id="formTipoEntrega">
                        @csrf
                        ...
                        <div class="form-group mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <select
                                    name="estado"
                                    id="estado"
                                    class="form-control premium-input @error('estado') is-invalid @enderror"
                                    required
                                >
                                    <option value="" style="background-color: #0D1324;">-- Seleccione --</option>
                                    <option value="1" {{ old('estado') == '1' ? 'selected' : '' }} style="background-color: #0D1324;">Activo</option>
                                    <option value="0" {{ old('estado') === '0' ? 'selected' : '' }} style="background-color: #0D1324;">Inactivo</option>
                                </select>
                                @error('estado')
                                    <span class="invalid-feedback font-weight-bold mt-2" style="color: #F87171;"><i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}</span>
                                @enderror
                                        ...
                                        <td class="text-center">
                                            <div class="d-inline-flex" style="gap: 6px;">
                                                ...
                                                <button type="button" 
                                                        ...
                                                        title="Editar"
                                                        onclick="prepararEdicion({{ $item->id }}, '{{ addslashes($item->nombre) }}', '{{ addslashes($item->descripcion ?? '') }}', '{{ $item->getRawOriginal('estado') ?? ($item->estado ? '1' : '0') }}')">
                                                    <i class="fas fa-pen fa-sm"></i>
                                                </button>
                                                
                                                {{-- Formulario Eliminar Seguro --}}
...
<script>
    ...
    function resetFormulario() {
        ...
        const form = document.getElementById('formTipoEntrega');
        form.action = "{{ route('admin.tipo-entrega.store') }}";
